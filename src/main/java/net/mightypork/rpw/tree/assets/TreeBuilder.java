...
public class TreeBuilder {
    ...
    private AssetTreeGroup rootGroup;
        ...
        for (final AssetEntry ae : Sources.vanilla.getAssetEntries()) {
            if (! fullTree) {
                ...
                if (! Config.SHOW_TEXTS) {
                    if (ae.getType() == EAsset.TEXT) {
                        continue; // skip texts
                    }
                }

                if (! Config.SHOW_TEXTURES) {
                    if (ae.getType() == EAsset.IMAGE) {
                ...
                if (! Config.SHOW_OBSOLETE_DIRS) {
                    boolean del = ae.getKey().startsWith("assets.minecraft.sound") && ! ae.getKey().startsWith("assets.minecraft.sounds");
                    // sound dir
                ...
                if (! Config.SHOW_SOUNDS) {
                    if (ae.getType() == EAsset.SOUND) {
                        continue; // skip sounds
                    }
                }
            }
